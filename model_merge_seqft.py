...
def main():
    ...
    for data_id in li_data_id:
        model_id='outputs/llama3-8b-ft_{}_round1/checkpoint-{}/'.format(data_id,18516 if data_id=='metamath4' else 780)
        models[data_id]=AutoModelForCausalLM.from_pretrained(model_id,torch_dtype='auto')
    ...
    for j,data_id in enumerate(li_data_id):
        ...
        output_dir='outputs/merged-llama3-8b-ft_round1_{}'.format( '+'.join([ li_data_id[i] for i in range(j+1) ]) )
